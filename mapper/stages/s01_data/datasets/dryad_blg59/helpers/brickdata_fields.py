...
def _match_role_overrides(*, key: str, label: str, brick_class: str | None, brick_class_local: str | None, cfg: dict[str, Any]) -> str | None:
    ...
    for rule in cfg["role_overrides"]:
        pattern = str(rule["pattern"]).strip()
        role = str(rule["role"]).strip().lower()
        source = str(rule["source"]).strip() if "source" in rule else "key"
...
def _match_role_from_rules(*, key: str, label: str, brick_class: str | None, brick_class_local: str | None, cfg: dict[str, Any]) -> str | None:
    ...
    for rule in cfg["role_rules"]:
        pattern = str(rule["pattern"]).strip()
        role = str(rule["role"]).strip()
        source = str(rule["source"]).strip() if "source" in rule else "brick_class_local"
        if pattern and role and re.search(pattern, haystacks.get(source, haystacks["brick_class_local"]), flags=re.IGNORECASE):
            return role
    class_roles = cfg.get("class_roles", {})
...
def _unit_candidates(*, key: str, label: str, class_local: str | None, brick_class: str | None, cfg: dict[str, Any]) -> list[str]:
    rules = cfg["unit_candidates_tokens"] if "unit_candidates_tokens" in cfg else cfg["unit_candidates"]
    hay = " ".join([str(key), str(label), str(class_local or ""), str(brick_class or "")]).upper()
    out: list[str] = []
    seen: set[str] = set()
    ...
    for rule in cfg.get("unit_candidates_overrides", []):
        source = str(rule["source"]) if "source" in rule else "key"
        pattern = str(rule["pattern"]).strip()
        haystacks = {
            "key": str(key),
            "label": str(label),
...
def get_brick_fields(brick_row: dict[str, Any], brick_cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    role = _match_role_overrides(key=key, label=label, brick_class=brick_class, brick_class_local=class_local, cfg=brick_cfg)
    if not role:
        role = _match_role_from_rules(key=key, label=label, brick_class=brick_class, brick_class_local=class_local, cfg=brick_cfg)
    if not role:
        role = _match_role_fallback(key=key, label=label)
    if not role:
        role = str(brick_cfg["default_role"]).strip().lower()
    candidates = _unit_candidates(key=key, label=label, class_local=class_local, brick_class=brick_class, cfg=brick_cfg)
    return {
        "label": label,
