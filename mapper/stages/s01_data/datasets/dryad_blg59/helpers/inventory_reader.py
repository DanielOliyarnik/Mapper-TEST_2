...
def _exclude_key(key: str, drop_patterns: list[str], drop_cerc: bool) -> bool:
    patterns = list(drop_patterns or [])
    if drop_cerc:
...
def read_inventory(input_dir: Path, cfg: dict[str, Any], ingest_cfg: dict[str, Any]):
    import pandas as pd
    ...
    for spec in list(ingest_cfg["files"]):
        ...
        drop_patterns = list(spec.get("drop_columns") or EXCLUDE_PATTERNS)
